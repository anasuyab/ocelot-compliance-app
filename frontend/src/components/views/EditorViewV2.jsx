...
const EditorViewV2 = ({ theme = 'default', blueprintImage, initialRooms = [], onComplete, imageMetadata = null }) => {
  // --- State ---
  ...
  // Generate color map from initial rooms
  React.useEffect(() => {
    if (initialRooms && initialRooms.length > 0) {
      const uniqueTypes = [...new Set(initialRooms.map(r => r.type?.toLowerCase()))];
      const newColorMap = {};
  ...
  // --- Helper: Convert API room format to editor format ---
  const convertRoomToEditorFormat = (room, scale) => {
    ...
    if ((shapeType === 'rect' || room.coords?.x !== undefined) && room.coords) {
      return {
        ...
        dimensions: room.dimensions || `${room.coords.w}px x ${room.coords.h}px`
      };
    }
    ...
    if ((shapeType === 'circle' || room.coords?.cx !== undefined) && room.coords) {
      return {
        ...
        color: getColorForRoomType(room.type),
        dimensions: room.dimensions || `${room.coords.r * 2}px Dia`
      };
    }
    ...
    if ((shapeType === 'polygon' || room.points) && room.points) {
      return {
        ...room,
        type: 'polygon',
        points: room.points.map(([x, y]) => [x * scale, y * scale]),
        color: getColorForRoomType(room.type),
        dimensions: room.dimensions || 'Irregular'
      };
    }

    // Fallback: Create a placeholder rect
    console.warn('Room has no geometry data, creating placeholder:', room.name);
    return {
      ...room,
      type: 'rect',
      coords: { x: 100 * scale, y: 100 * scale, w: 100 * scale, h: 100 * scale },
      color: getColorForRoomType(room.type),
      dimensions: '100px x 100px'
    };
  };
  
  // Helper to get color for room type (now uses dynamic color map)
  const getColorForRoomType = (type) => {
    return colorMap[type?.toLowerCase()] || 'rgba(128, 128, 128, 0.5)';
  };
  ...
  const onImageLoad = (event) => {
    ...
    console.log('EditorView - Image dimensions:', { 
    ...
    if (!initialRoomsScaled && initialRooms.length > 0) {
      const scaledRooms = initialRooms.map(room => convertRoomToEditorFormat(room, scale));
      
      setRooms(scaledRooms);
      setInitialRoomsScaled(true);
      console.log('EditorView - Converted and scaled rooms:', scaledRooms);
    }
  };
